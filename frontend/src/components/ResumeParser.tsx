// src/components/ResumeParser.tsx
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-xl border border-zinc-300 bg-white px-3 py-2 text-xs font-medium text-zinc-900 shadow-sm hover:shadow"
          disabled={busy}
        >
          {busy ? "Parsing…" : "Upload"}
        </button>
      ...
      {error ? (
        ...
        <div className="mt-3 text-xs text-zinc-600">Tip: Clean PDFs parse best; photos also work.</div>
      )}
    </div>
  );
}
